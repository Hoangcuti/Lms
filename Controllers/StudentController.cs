...
public class StudentController : Controller
{
    ...
    public async Task<IActionResult> Profile()
    {
        ...
        return Json(new
        {
            ...
            jobTitle = user.JobTitle?.TitleName,
    ...
    public async Task<IActionResult> AddLessonComment(int lessonId, [FromBody] AddCommentDto dto)
    {
        ...
        var user = await _db.Users.FindAsync(userId.Value);
        return Ok(new { success = true, id = comment.Id, userName = user?.FullName ?? "B?n" });
    }

    // ============================================================
    // API: S? KI?N ��O T?O OFFLINE
    // ============================================================
    ...
    public async Task<IActionResult> RegisterOfflineEvent(int eventId)
    {
        ...
        if (ev.AttendanceLogs.Any(a => a.UserId == userId.Value))
            return BadRequest(new { error = "Ban da dang ky su kien nay roi." });

        if (ev.MaxParticipants.HasValue && ev.AttendanceLogs.Count >= ev.MaxParticipants.Value)
            return BadRequest(new { error = "Su kien da du so nguoi tham gia." });
